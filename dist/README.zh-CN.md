...
| 天线灯 | 它在干嘛 | 什么意思 |
|---|---|---|
| 红灯急闪 | 双手举过头顶在抖，头顶冒气泡 | 有 session 卡着超过 60 秒了。气泡写着是哪个 project、卡在哪条命令 |
| 黄灯脉冲 | 举起手，屏幕转成警告 | 有 session 在等你授权 |
| 黄灯脉冲 | 歪着头，旁边浮一个问号 | 有 session 在等你回答——要的是你打字，不是一个决定 |
| 短暂闪黄 | 屏幕转成警告，人继续干活 | 有个工具被打断了。通常不等于任务失败 |
| 绿灯慢闪 | 手在键盘上。改文件时敲键盘，读代码时凑近屏幕，等命令跑完时慢下来看着 | 有 session 在干活 |
| 绿灯常亮 | 手左右扫着整理，屏幕在重排 | 正在压缩上下文 |
| 绿灯慢脉冲 | 手从键盘上拿开，看着屏幕 | 它派了个后台 agent 就把这一轮结束了。等 agent 跑完它自己会再开口，所以这一轮不报完成 |
| 短暂绿闪 | 点两下头 | 刚干完一轮。不代表活干对了 |
| 灭 | 趴桌上睡着，头顶飘 zzz | 全都停了 |

**左键点它**展开列表，看每个 session 在忙什么、多久了。
**点列表里带 ↗ 的行**，直接跳回那个 session 所在的终端标签页（支持 Orca 和 iTerm2）。
**鼠标停在某一行**，气泡显示这个 session 在哪个目录、context 用了多少、这一轮跑了多久 vs 多久没动静、以及最后一次工具调用。**停在机器人身上**则显示额度还剩多少、什么时候回血。
**鼠标移到某一行，点行尾的 ×** 把它静音——不在列表里显示，也不影响宠物表情。下次你在那个 session 里敲字它自动回来。
**右键宠物**出菜单：暂停 / 低动态 / 连接状态 / 演示 / 开机自启 / 退出。**右键某一行**：给这个 session 起名、置顶、看最近活动、静音。
**拖动**换位置，会记住。
...
2. `~/.claude/pet/` —— hook 程序、session 状态、完成记录和活动日志
3. **`~/.claude/settings.json` —— 追加 9 条 hook**
