...
| 等你回答 | 黄灯脉冲 | 歪着头，旁边浮一个问号（和等授权的举手区分开） |
| 正在压缩上下文 | 绿灯 | 手左右扫着整理，屏幕在重排而不是打印 |
...
两个来源永不混合。新鲜的那个赢，因为一个更权威但过期的读数描述的是可能已经翻篇的窗口。**注意：statusline 那个 `rate_limits` 的结构没有对着真实 payload 验证过**——解析器认几种可能的拼写，一种都认不出就返回空，而不是给你一个错数字。
