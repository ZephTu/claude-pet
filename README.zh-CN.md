...
另一个内置形象是机器人，它天线上那颗灯是这些状态最好认的一种表达，所以下面这张表是照着它写的。猫用贴图表达同一批状态，**右键 → Appearance** 可以换，下面讲形象那段列了各自能说什么、不能说什么。

天线顶上那颗灯不用聚焦，余光扫过去就能读到：
...
**Robot** 是一张 SVG，每个状态就是一条 CSS 规则。它保留了猫做不到的一件事：灯加姿势能分开**每一个** busy 状态，包括改文件和等命令跑完。你要是更看重这个区分而不是画风，就选它。
...
**停在宠物身上 1 秒**，气泡把额度画成两条进度条——五小时和一周并排，各带倒计时。超过 60% 变黄、超过 85% 变红，和天线灯是同一套预警色。
