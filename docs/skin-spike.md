...
三样局部形变，没用新素材：尾巴绕根部转、眼眶纵向压扁当眨眼、瞳孔小幅平移。
每张贴图的尾巴根部和眼眶位置是量出来的——眼睛用暗像素连通域（每只约 1100 像素），
目测把 working 那张读高了 0.14，第一版压的是鼻子。

对比页：`docs/previews/cat-life.html`（贴图 base64 内嵌，双击就能开；引用 PNG 的话
WebGL 会拒绝 file:// 的贴图，而且静默失败什么都不画）。改参数用
`scripts/make-cat-life-preview.py` 重新生成。

**眨眼走自己的时钟。** `time` 每次 setState 归零（"刚干完一轮"的弹跳要的就是
"距这轮结束多少秒"），共用的话下次眨眼被排在一个刚归零的时钟之后，而宠物几秒
换一次状态——它永远不会眨。
